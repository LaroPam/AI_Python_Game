...
npm install
npm start
# откройте http://localhost:8080/game/index.html если http-server поднялся из корня
```

Игра стартует из `index.html` без сборки.

## Структура
- `main.js` — точка входа, подгрузка Phaser с CDN.
...
- `assets/` — placeholder-папки для ассетов (генерируются плейсхолдеры через `Graphics.generateTexture`).
